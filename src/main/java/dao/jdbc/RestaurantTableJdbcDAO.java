...
public class RestaurantTableJdbcDAO implements RestaurantTableDAO {
    ...
    private RestaurantTable map(ResultSet rs) throws SQLException {
        ...
        t.setStatus(model.TableStatus.valueOf(rs.getString("status")));
    ...
    public void deleteById(Long id) {
        try (Connection c = Db.getConnection();
             PreparedStatement ps = c.prepareStatement("DELETE FROM dining_tables WHERE id=?")) {
            ps.setLong(1, id);
            ps.executeUpdate();
        } catch (SQLException ex) { throw new RuntimeException(ex); }
    }
    ...
    public Optional<RestaurantTable> findById(Long id) {
        try (Connection c = Db.getConnection();
             PreparedStatement ps = c.prepareStatement(
                     "SELECT id, table_no, status, note FROM dining_tables WHERE id=?")) {
            ps.setLong(1, id);
    ...
    public Optional<RestaurantTable> findByTableNo(int tableNo) {
        try (Connection c = Db.getConnection();
             PreparedStatement ps = c.prepareStatement(
                     "SELECT id, table_no, status, note FROM dining_tables WHERE table_no=?")) {
            ps.setInt(1, tableNo);
            try (ResultSet rs = ps.executeQuery()) {
                return rs.next() ? Optional.of(map(rs)) : Optional.empty();
            }
    ...
    public List<RestaurantTable> findAll(int offset, int limit) {
        ...
        try (Connection c = Db.getConnection();
             PreparedStatement ps = c.prepareStatement(
                     "SELECT id, table_no, status, note FROM dining_tables ORDER BY table_no LIMIT ? OFFSET ?")) {
            ps.setInt(1, limit);
            ps.setInt(2, offset);
            try (ResultSet rs = ps.executeQuery()) {
    ...
    public List<RestaurantTable> findByStatus(TableStatus status) {
        ...
        try (Connection c = Db.getConnection();
             PreparedStatement ps = c.prepareStatement(
                     "SELECT id, table_no, status, note FROM dining_tables WHERE status=? ORDER BY table_no")) {
    ...
    public void setNote(Long tableId, String note) {
        try (Connection c = Db.getConnection();
             PreparedStatement ps = c.prepareStatement(
                     "UPDATE dining_tables SET note=?, updated_at=NOW() WHERE id=?")) {
            ps.setString(1, note);
            ps.setLong(2, tableId);
            ps.executeUpdate();
        } catch (SQLException ex) { throw new RuntimeException(ex); }
